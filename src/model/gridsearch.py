...
def gridsearch(src: str, target: str, scenario: str, model_type: str, iteration: int):
    ...
    hyper_params = dvc.api.params_show(
        os.path.join(os.path.dirname(__file__), "..", "..", "src/model/gridsearch.yaml")
    )
    ...
    if model_type == "IForest":
        ...
        for i in range(0, iteration):
            model = IForest(
                ...
                contamination=random.choice(contamination),
            ...
            with open(
                target + "/" + scenario + "/gridsearch_" + model_type + ".txt", "a"
            ) as outfile:
    ...
    if model_type == "LOF":
        ...
        for i in range(0, iteration):
            model = LocalOutlierFactor(
                n_neighbors=random.choice(n_neighbors),
                algorithm=algorithm,
                leaf_size=random.choice(leaf_size),
                p=random.choice(p),
                contamination=random.choice(contamination),
    ...
    if model_type == "VAE":
        ...
        for i in range(0, iteration):
            model = VAE(
                ...
                l2_regularizer=random.choice(l2_regularizer),
                validation_size=random.choice(validation_size),
